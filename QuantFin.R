...
data.rfr <-read.zoo("RFR.csv",sep=",", header = TRUE, format = "%Y-%m-%d")
...
rfr.returns <- diff(data.rfr) / lag(data.rfr, k = -1)  
...
rfr.return.oneyear <- window(rfr.returns,end=calcdate,start=calcdateoney)
...
fund.sharpe <- (mean(fund.returns.oneyear)-mean(rfr.return.oneyear))/sd(fund.returns.oneyear)
bench.sharpe <- (mean(benchmark.returns.oneyear)-mean(rfr.return.oneyear))/sd(benchmark.returns.oneyear)
...
fund.traynor <- (mean(fund.returns.oneyear)-mean(rfr.return.oneyear))/beta
benchmark.traynor <- (mean(benchmark.returns.oneyear)-mean(rfr.return.oneyear))/1

#maximum drawdown
data.fund.oneyear[which.max(data.fund.oneyear)]
data.fund.oneyear[which.min(data.fund.oneyear)]
(max(data.fund.oneyear)/min(data.fund.oneyear)-1)*100
